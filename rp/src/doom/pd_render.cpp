...
static uint16_t patch_decoder_circular_buf[PATCH_DECODER_CIRCULAR_BUFFER_SIZE] __cart_app_free("pdcbuf"); /* MD/DOOM */
...
static uint8_t patch_decoder_tmp[256 * WHD_MAX_COL_UNIQUE_PATCHES] __cart_app_free("pdtmp"); /* MD/DOOM */
...
extern uint8_t *const frame_buffer[2]; /* MD/DOOM: both the one 320x200 buffer */
static uint8_t __aligned(4) visplane_bit[(SCREENWIDTH / 8) * MAIN_VIEWHEIGHT] __cart_app_free("visplane_bit"); /* MD/DOOM: parked in the cart hole */ // this is also used for patch decoding in core1 (since flats are done by then)
...
static __aligned(4) int16_t column_heads[SCREENWIDTH * 2] __cart_app_free("colheads"); /* MD/DOOM */
...
#endif
    /* MD/DOOM: Core 1 is the framework's job server; hand it this frame's
...
void maybe_draw_single_screen(int patch_num) {
    /* MD/DOOM: one buffer, composited synchronously, so a page is redrawn
     * whole every frame rather than split over two and then left alone. */
    next_video_type = VIDEO_TYPE_SINGLE;
    draw_splash(patch_num, 0, SCREENHEIGHT, frame_buffer[0]);
    sub_gamestate = 2;
}
...
//    ST_Drawer(false, refresh);
    ...
    I_VideoBuffer = frame_buffer[frame]; /* MD/DOOM */
    V_RestoreBuffer();
        ...
        switch (wipestate) {
            case WIPESTATE_NONE: {
                if (wipe_start) {
                    ...
                    uint32_t base;
#if PICO_ON_DEVICE
                    base = (uintptr_t) frame_buffer[0]; /* MD/DOOM */
#else
                    base = 0;
#endif
...
/* MD/DOOM: pd_start_save_pause / pd_end_save_pause removed (no flash saves). */
