...
/* MD/DOOM: the WHX is whichever level pack is in the PACK_FLASH window. */
#include "pack.h"
#define wad_map_base (pack_base())
...
static wad_file_t fileo = {
        ...
        .mapped = NULL, /* MD/DOOM: set at open */
        .path = "<here>",
};

static wad_file_t *W_Memory_OpenFile(const char *path)
{
    whd_map_base = wad_map_base; /* MD/DOOM */
    fileo.mapped = wad_map_base;
#if !USE_WHD
    ...
    if (fileo.mapped[0] != 'I' || fileo.mapped[1] != 'W' || fileo.mapped[2] != 'H' || fileo.mapped[3] != 'X') {
        panic("No WHX in the pack window"); /* MD/DOOM */
    }
#else
