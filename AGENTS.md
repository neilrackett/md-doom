...
- `tools/levelpack.py` + `tools/whd_gen/` — the offline pack builder;
  `tools/png_to_mono.py` — the backdrop converter.
...
1. **m68k boot stubs 4 IRQ vectors** — HBL (`$68`), Timer-A (`$134`), Timer-C (`$114`) and Timer-D (`$110`) all point at a 1-instruction `userfw_dummy_irq` (just `rte`). MFP Timer A/C/D are disabled+masked at IERA/IERB so they never fire; HBL is masked by SR=$2300. Timer-B (`$120`) is owned by the YM audio path; it is left idle when STE DMA sound is in use.
...
5. **RP side.** `audio_render_frame()` paces to ~50 Hz and asks the installed fill callback for exactly the current mode's byte count, writing into the 1 KB cart buffer at `CART_AUDIO_BUFFER_OFFSET` (`$FA4100`). MD/DOOM installs `doomapp_audio_fill` → `doom_sound_fill`.
...
- `fb_chunked.c` / `fb_chunked.h` — `fb_chunked_buffer` (64 KB, also Doom's video buffer) + the Core 1 job dispatcher (`fb_core1_dispatch` / `fb_core1_wait`, one dispatch per wait, never straddling a publish) + `fb_core1_park` / `fb_core1_unpark` (hold Core 1 in RAM while flash is programmed). **Between jobs Core 1 refills the cart audio buffer** (`audio_render_frame()` in `fb_core1_loop`), so sound keeps up however long a frame takes on Core 0; the main loop no longer calls it.
...
4×4 Bayer (default), 4×4 clustered-dot. A colour is drawn as its nearest
...
- `md/i_input.c` — `I_GetEvent` = `ikbd_clear_command` + `fb_pump_rom3` +
  ...
  = Ctrl/Space/Alt/Shift, Start = Esc, Select = Tab). ESC auto-exit is off;
  quitting is `I_Quit` → `ikbd_request_boot_gem()`.
...
- `md/i_mdsound.c` — upstream's channel model and ADPCM decoder with the
  ...
  on Core 1 (`fb_core1_loop` idle / `SafeUpdateSound`).
