...
3b. **The m68k heartbeats once per VBL** at `RELOC_WINDOW_BASE + protocol` (`$FB8E00`), from the display loop and therefore only ever from the relocated copy in ST RAM. One read carries three facts: this firmware relocated itself, so the cartridge code area can be reclaimed; the ST is still alive; and which protocol version to expect. `emul_start` blocks on the first one (`fb_wait_st_reloc`, 15 s) straight after `commemul_init` and before anything could write into that area, and logs which branch it took — a boot without the signal (no ST, older image, Shift held for the desktop) simply runs without the reclaim. This is the only point at which the RP can know the ST exists: `fb_wait_blit_ack` times out rather than blocking.
...
- `src/main.s` — m68k cartridge header + boot dispatcher. Lives at `$FA0000` (ROM4 cartridge region). One header entry carries both jobs: `CA_INIT` (bit 27) autostarts at boot, and `CA_RUN` is what the desktop calls when the cartridge's `MDDOOM.TOS` icon is double-clicked — TOS shows the cartridge as a lower-case `c` drive and lists every `CA_NEXT` chain entry by name, so a launchable file needs no filesystem emulation and no RP-side code. **`CA_RUN` must never be 0**: the desktop calls it for whatever the user clicks, and 0 is a jsr to address 0. `pre_auto` runs in supervisor mode, copies `start_rom_code` below ST screen memory and jumps there; that copy checks resolution (high-res falls to GEM with a warning), checks `phystop` for the 512 KB the screen pages assume, prints a banner, and skips to GEM if either Shift key is held — the desktop route, which is what lets an AUTO-folder Xpad provider install itself before MD/DOOM takes the machine. Then it **relocates userfw into ST RAM and jumps there** rather than running it in place from the cartridge: the image carries its own header (a `bra.w` to the entry at +0, `'MDOM'` at +4, its length at +8), main.s checks the magic at both ends of the copy and refuses a bad one, and the destination is `UFW_RAM_DEST` = `$6E000`, an 8 KB window immediately below screen page A that userfw already owns and GEMDOS has nothing in at `CA_INIT` time. A `fail` directive in userfw.s breaks the build if the image outgrows the window, which would otherwise overrun the screen silently (verified by building with the window shrunk). This costs nothing in userfw.s, which is linked at `$000800` and has always been position-independent, and it is what allows the cartridge's 16 KB code area to become ordinary SRAM.
- **Atari RAM**: userfw runs from `$6E000` and owns `$6E000..$80000` (its relocated code, then the two screen pages and their tails). Its 32-byte vector/MFP save area is `UFW_SAVE` = `$77FA0`; it used to hang off `physbase - 32`, which is the top of the GEMDOS pool on a 1 MB machine and therefore unsafe once the game can be launched from the desktop. Beyond that the cartridge code uses none of the ST's RAM (no `.bss`, no heap). Data lives in registers or the shared region. Adding naive BSS to `userfw.s` will silently corrupt system variables — keep state in `APP_FREE` / `SHARED_VARIABLES`, the cart code itself (which is **read-only** from m68k — writes bus-error), or registers.
- **Xpad** (`XPAD_ENABLE`, on by default since v0.7.0). Two things had to be true before it could work. A provider has to exist when the cookie walk runs, which it never does at `CA_INIT` bit 27 — before the AUTO folder — so the desktop launch is the route that makes it possible. And a provider has to keep running while userfw owns the machine: TOS's 200 Hz Timer-C is dead, so `etv_timer` ($400), which md-sidepad's provider hooks by default, is never called. userfw now calls that vector itself, **four times per VBL** — it is a 200 Hz vector and such hooks divide by four, so one call a frame would leave the pad at 12.5 Hz. **A0 is the hazard**: it is the Timer-B audio cursor, live across the whole frame, and an `etv_timer` handler may clobber it by convention, so the burst saves A0 and masks MFP (the MFP latches Timer-B rather than losing it, so the YM path pays phase jitter rather than garbage; on the DMA path Timer-B is idle and A0 is free). None of it runs unless a provider was found. **Not supported**: providers that hook the VBL autovector at `$70` (TOS's handler sets the screen base from `_screenpt` and would fight the page flipping) or that pace off `_hz_200`, which is frozen.
...
- `fb_font.c` — text into the chunked buffer. Note `font_set_color` masks to 4 bits (the framework's 16-colour assumption), so on the Doom path text can only use PLAYPAL indices 0..15 — the test palette keeps a grey ramp there for that reason.
...
smaller than 16), C64, ZX Spectrum and PICO-8 palettes. The five fixed
palettes match differently from the rest (`vivid_pair`): Doom's colours
are mostly dark browns and greys, and by plain distance nearly all of
them are nearest black, so the game was black with a sprinkle of colour.
Instead the pen is chosen by hue alone (colour and pens scaled to full
brightness), the colour's brightness is stretched with a gamma of 0.75,
and the dither runs along a brightness ladder of black plus every pen of
that hue (EGA's dark and bright pairs, C64's greys). Checked on the host
against a Python prototype of the same arithmetic. Dither modes:
